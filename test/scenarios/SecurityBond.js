...
module.exports = function (glob) {
	...
	describe.skip("SecurityBond", function () {
		...
		it(testCounter.next() + ": MUST SUCCEED [depositTokens]: User A adds 10 tokens to deposited balance", async() => {
			...
			try {
				await glob.web3SecurityBond.depositTokens(glob.web3Erc20.address, 10, { from: glob.user_a });
			}
			catch (err) {
				assert(false, 'This test must succeed. [Error: ' + err.toString() + ']');
			}
		...
		it(testCounter.next() + ": MUST FAIL [depositTokens]: Cannot be called with zero address", async() => {
			try {
				await glob.web3SecurityBond.depositTokens(0, 5, { from: glob.user_a });
				assert(false, 'This test must fail.');
			}
			catch (err) {
				assert(err.toString().includes('revert'), err.toString());
		...
		it(testCounter.next() + ": MUST FAIL [depositTokens]: Cannot be called with zero amount", async() => {
			try {
				await glob.web3SecurityBond.depositTokens(glob.web3Erc20.address, 0, { from: glob.user_a });
				assert(false, 'This test must fail.');
			}
			catch (err) {
				assert(err.toString().includes('revert'), err.toString());
		...
		it(testCounter.next() + ": MUST FAIL [depositTokens]: User does not have enough tokens to deposit.", async() => {
			...
			try {
				await glob.web3SecurityBond.depositTokens(glob.web3Erc20.address, 9999, { from: glob.user_a });
				assert(false, 'This test must fail.');
			}
			catch (err) {
				assert(err.toString().includes('revert'), err.toString());
		...
		it(testCounter.next() + ": MUST FAIL [depositCount]: Cannot be called from non-owner address", async() => {
			try {
				await glob.web3SecurityBond.depositCount(glob.user_a, { from: glob.user_a });
				assert(false, 'This test must fail.');
			}
			catch (err) {
				assert(err.toString().includes('revert'), err.toString());
		...
		it(testCounter.next() + ": MUST SUCCEED [activeBalance]: Total should be 2.0 ETH", async() => {
			try {
				let balance = await glob.web3SecurityBond.activeBalance(0);
		...
		it(testCounter.next() + ": MUST SUCCEED [activeBalance]: Total should be 10 tokens", async() => {
			try {
				let balance = await glob.web3SecurityBond.activeBalance(glob.web3Erc20.address);
		...
		it(testCounter.next() + ": MUST SUCCEED [stagedBalance]: 0 ETH for User A", async() => {
			try {
				let balance = await glob.web3SecurityBond.stagedBalance(glob.user_a, 0);
		...
		it(testCounter.next() + ": MUST SUCCEED [stagedBalance]: 0 tokens for User A", async() => {
			try {
				let balance = await glob.web3SecurityBond.stagedBalance(glob.user_a, glob.web3Erc20.address);
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawEthers]: User A wants to withdraw 1 ETH but thay are NOT staged", async() => {
			try {
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_a, 0);
				let oldEthersBalance = await web3.eth.getBalancePromise(glob.user_a);

				let result = await glob.web3SecurityBond.withdrawEthers(web3.toWei(1, 'ether'), { from: glob.user_a });
				...
				let newStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_a, 0);
		...
		it(testCounter.next() + ": MUST FAIL [stage]: UnitTestHelpers_FAIL stages 0.5 ETH to User A", async() => {
			try {
				await glob.web3UnitTestHelpers_FAIL_TESTS.callToStage_SECURITYBOND(glob.web3SecurityBond.address, web3.toWei(0.5, 'ether'), 0, glob.user_a);
				assert(false, 'This test must fail.');
			}
			catch (err) {
				assert(err.toString().includes('revert'), err.toString());
		...
		it(testCounter.next() + ": MUST SUCCEED [stage]: UnitTestHelpers_SUCCESS stages 0.5 ETH to User C", async() => {
			try {
				let oldActiveBalance = await glob.web3SecurityBond.activeBalance(0);
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_c, 0);

				await glob.web3UnitTestHelpers_SUCCESS_TESTS.callToStage_SECURITYBOND(glob.web3SecurityBond.address, web3.toWei(0.5, 'ether'), 0, glob.user_c);

				let newActiveBalance = await glob.web3SecurityBond.activeBalance(0);
				let newStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_c, 0);
		...
		it(testCounter.next() + ": MUST SUCCEED [stage]: UnitTestHelpers_SUCCESS stages 2 tokens to User C", async() => {
			try {
				let oldActiveBalance = await glob.web3SecurityBond.activeBalance(glob.web3Erc20.address);
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_c, glob.web3Erc20.address);

				await glob.web3UnitTestHelpers_SUCCESS_TESTS.callToStage_SECURITYBOND(glob.web3SecurityBond.address, 2, glob.web3Erc20.address, glob.user_c);

				let newActiveBalance = await glob.web3SecurityBond.activeBalance(glob.web3Erc20.address);
				let newStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_c, glob.web3Erc20.address);
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawEthers]: User C withdraw 1.0 ETH (but only can get 0.5)", async() => {
			try {
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_c, 0);
				let oldEthersBalance = await web3.eth.getBalancePromise(glob.user_c);

				let result = await glob.web3SecurityBond.withdrawEthers(web3.toWei(1.0, 'ether'), { from: glob.user_c });
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawTokens]: User C withdraw 10 tokens (but only can get 2)", async() => {
			try {
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_c, glob.web3Erc20.address);
				let oldTokensBalance = await glob.web3Erc20.balanceOf(glob.user_c);

				await glob.web3SecurityBond.withdrawTokens(10, glob.web3Erc20.address, { from: glob.user_c });
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawEthers]: Repeat user C withdraw 1.0 ETH (but now he will get nothing)", async() => {
			try {
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_c, 0);
				let oldEthersBalance = await web3.eth.getBalancePromise(glob.user_c);
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawTokens]: Repeat user C withdraw 10 tokens (but now he will get nothing)", async() => {
			try {
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.user_c, glob.web3Erc20.address);
				let oldTokensBalance = await glob.web3Erc20.balanceOf(glob.user_c);
		...
		it(testCounter.next() + ": MUST SUCCEED [stage]: UnitTestHelpers_SUCCESS stages 0.2 ETH to Owner", async() => {
			try {
				let oldActiveBalance = await glob.web3SecurityBond.activeBalance(0);
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.owner, 0);

				await glob.web3UnitTestHelpers_SUCCESS_TESTS.callToStage_SECURITYBOND(glob.web3SecurityBond.address, web3.toWei(0.2, 'ether'), 0, glob.owner);

				let newActiveBalance = await glob.web3SecurityBond.activeBalance(0);
				let newStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.owner, 0);
		...
		it(testCounter.next() + ": MUST SUCCEED [stage]: UnitTestHelpers_SUCCESS stages 2 tokens to Owner", async() => {
			try {
				let oldActiveBalance = await glob.web3SecurityBond.activeBalance(glob.web3Erc20.address);
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.owner, glob.web3Erc20.address);

				await glob.web3UnitTestHelpers_SUCCESS_TESTS.callToStage_SECURITYBOND(glob.web3SecurityBond.address, 2, glob.web3Erc20.address, glob.owner);

				let newActiveBalance = await glob.web3SecurityBond.activeBalance(glob.web3Erc20.address);
				let newStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.owner, glob.web3Erc20.address);
		...
		it(testCounter.next() + ": MUST SUCCEED [stage]: After 2 seconds UnitTestHelpers_SUCCESS stages other 0.3 ETH to Owner", async() => {
			try {
				...
				await glob.web3UnitTestHelpers_SUCCESS_TESTS.callToStage_SECURITYBOND(glob.web3SecurityBond.address, web3.toWei(0.3, 'ether'), 0, glob.owner);
		...
		it(testCounter.next() + ": MUST SUCCEED [stage]: Also UnitTestHelpers_SUCCESS stages other 3 tokens to Owner", async() => {
			try {
				...
				await glob.web3UnitTestHelpers_SUCCESS_TESTS.callToStage_SECURITYBOND(glob.web3SecurityBond.address, 3, glob.web3Erc20.address, glob.owner);
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawEthers]: Owner wants to withdraw 1.0 ETH before timeout but will get 0", async() => {
			try {
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.owner, 0);
				let oldEthersBalance = await web3.eth.getBalancePromise(glob.owner);

				let result = await glob.web3SecurityBond.withdrawEthers(web3.toWei(1.0, 'ether'), { from: glob.owner });
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawTokens]: Onwer withdraw 10 tokens before timeout but will get 0", async() => {
			try {
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.owner, glob.web3Erc20.address);
				let oldTokensBalance = await glob.web3Erc20.balanceOf(glob.owner);

				await glob.web3SecurityBond.withdrawTokens(10, glob.web3Erc20.address, { from: glob.owner });
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawEthers]: After 8 secs, Owner withdraw 1.0 ETH (but only can get 0.2)", async() => {
			try {
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawTokens]: Onwer withdraw 10 tokens (but only can get 2)", async() => {
			try {
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.owner, glob.web3Erc20.address);
				let oldTokensBalance = await glob.web3Erc20.balanceOf(glob.owner);
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawEthers]: After other 3 secs, Owner withdraw 1.0 ETH (but only can get 0.3)", async() => {
			try {
				await Helpers.sleep(8000);
		...
		it(testCounter.next() + ": MUST SUCCEED [withdrawTokens]: Onwer withdraw 10 tokens (but only can get 3)", async() => {
			try {
				let oldStagedBalance = await glob.web3SecurityBond.stagedBalance(glob.owner, glob.web3Erc20.address);
				let oldTokensBalance = await glob.web3Erc20.balanceOf(glob.owner);
