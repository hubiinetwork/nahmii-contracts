...
module.exports = function (glob) {
	...
	describe("SecurityBond", function () {
		...
		it(testCounter.next() + ": MUST SUCCEED [payable]: add 0.5 Ethers to user A deposited balance", async() => {
			try {
				await web3.eth.sendTransactionPromise({
						from: glob.user_a,
						to: glob.web3SecurityBond.address,
		...
		it(testCounter.next() + ": MUST SUCCEED [payable]: add 1.5 Ethers to user B deposited balance", async() => {
			try {
				await web3.eth.sendTransactionPromise({
						from: glob.user_b,
						to: glob.web3SecurityBond.address,
		...
		it(testCounter.next() + ": MUST SUCCEED [depositTokens]: 5 tokens added to A deposited balance", async() => {
			try {
				await glob.web3Erc20.approve(glob.web3SecurityBond.address, 5, { from: glob.user_a });
			}
			catch (err) {
				assert(false, 'This test must succeed. [Error: ' + err.toString() + ']');
			}
			try {
				await glob.web3SecurityBond.depositTokens(glob.web3Erc20.address, 5, { from: glob.user_a });
			}
			catch (err) {
		...
		it(testCounter.next() + ": MUST FAIL [depositTokens]: Cannot be called from owner address", async() => {
			try {
				await glob.web3SecurityBond.depositTokens(glob.web3Erc20.address, 5, { from: glob.owner });
				assert(false, 'This test must fail.');
			}
			catch (err) {
				assert(err.toString().includes('revert'), err.toString());
		...
		it(testCounter.next() + ": MUST SUCCEED [depositCount]: User B should have 1 deposit", async() => {
			try {
				let count = await glob.web3SecurityBond.depositCount(glob.user_b);
		...
		it(testCounter.next() + ": MUST SUCCEED [deposit]: User B should have 1.5 ETH at index 0", async() => {
			try {
				let args = await glob.web3SecurityBond.deposit(glob.user_b, 0);
		...
		it(testCounter.next() + ": MUST SUCCEED [deposit]: User A should have 5 tokens at index 1", async() => {
			try {
				...
				assert.equal(_amount, 5, "Unexpeced token deposit amount.");
				assert.notEqual(_timestamp, 0, "Timestamp cannot be null.");
			}
			catch (err) {
				assert(false, 'This test must succeed. [Error: ' + err.toString() + ']');
		...
		it(testCounter.next() + ": MUST SUCCEED [activeBalance]: Total should be 5 tokens", async() => {
			try {
				let balance = await glob.web3SecurityBond.activeBalance(glob.web3Erc20.address);
				assert.equal(balance, 5, 'Wrong balance [' + balance.toString() + ' tokens].');
			}
			catch (err) {
				assert(false, 'This test must succeed. [Error: ' + err.toString() + ']');
			}
		...
		it(testCounter.next() + ": MUST SUCCEED [setWithdrawalTimeout]: Set the withdrawal timeout to 0", async() => {
			try {
				await glob.web3SecurityBond.setWithdrawalTimeout(0);
			}
			catch (err) {
				assert(false, 'This test must succeed. [Error: ' + err.toString() + ']');
			}
