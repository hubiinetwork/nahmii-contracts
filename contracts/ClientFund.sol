...
contract ClientFund is Ownable, /*Configurable, */Beneficiary, Benefactor, AuthorizableServable, TransferControllerManageable {
    ...
    bytes32 public depositedBalanceTypeHash;
    bytes32 public settledBalanceTypeHash;
    bytes32 public stagedBalanceTypeHash;
    bytes32[] private allBalanceTypeHashes;

    bytes32 public depositTransactionTypeHash;
    bytes32 public withdrawalTransactionTypeHash;
    ...
    event SetWalletLockerEvent(WalletLocker walletLocker);
    ...
    onlyDeployer
    {
        ...
        depositedBalanceTypeHash = keccak256(abi.encodePacked(DEPOSITED_BALANCE_TYPE));
        settledBalanceTypeHash = keccak256(abi.encodePacked(SETTLED_BALANCE_TYPE));
        stagedBalanceTypeHash = keccak256(abi.encodePacked(STAGED_BALANCE_TYPE));

        allBalanceTypeHashes = new bytes32[](3);
        allBalanceTypeHashes[0] = depositedBalanceTypeHash;
        allBalanceTypeHashes[1] = settledBalanceTypeHash;
        allBalanceTypeHashes[2] = stagedBalanceTypeHash;

        depositTransactionTypeHash = keccak256(abi.encodePacked("deposit"));
        withdrawalTransactionTypeHash = keccak256(abi.encodePacked("withdrawal"));
    }
    ...
    /// @dev This function can only successfully be called once
    /// @param _walletLocker The (address of) WalletLocker contract instance
    function setWalletLocker(WalletLocker _walletLocker)
    public
    onlyDeployer
    {
        // Require that transaction tracker has not been set previously
        require(address(0) == address(walletLocker));

        // Set transaction tracker
        walletLocker = _walletLocker;

        // Emit event
        emit SetWalletLockerEvent(_walletLocker);
    }
    ...
    //    {
    //        return transactionTracker.getByIndex(wallet, depositTransactionTypeHash, index);
    //    }
    ...
    //        return transactionTracker.count(wallet, depositTransactionTypeHash);
    //    }
    ...
    //            wallet, depositTransactionTypeHash, currencyCt, currencyId, index
    //        );
    //    }
    ...
    //            wallet, depositTransactionTypeHash, currencyCt, currencyId
    //        );
    //    }
    ...
    //    returns (int256)
    //    {
    //        return balanceTracker.get(wallet, depositedBalanceTypeHash, currencyCt, currencyId);
    //    }
    ...
    //    returns (int256)
    //    {
    //        return balanceTracker.get(wallet, settledBalanceTypeHash, currencyCt, currencyId);
    //    }
    ...
    //    returns (int256)
    //    {
    //        return balanceTracker.get(wallet, stagedBalanceTypeHash, currencyCt, currencyId);
    //    }
    ...
    //    returns (int256)
    //    {
    //        return balanceTracker.get(wallet, depositedBalanceTypeHash, currencyCt, currencyId).add(
    //            balanceTracker.get(wallet, settledBalanceTypeHash, currencyCt, currencyId)
    //        );
    //    }
    ...
    returns (int256 amount, uint256 blockNumber)
    {
        ...
        //        0 < balanceTracker.logSize(wallet, depositedBalanceTypeHash, currencyCt, currencyId) ?
        //        balanceTracker.lastLog(
        //            wallet, depositedBalanceTypeHash, currencyCt, currencyId
        //        ) :
        //        (0, 0);
        //
        //        (int256 settledAmount, uint256 settledBlockNumber) =
        //        0 < balanceTracker.logSize(wallet, settledBalanceTypeHash, currencyCt, currencyId) ?
        //        balanceTracker.lastLog(
        //            wallet, settledBalanceTypeHash, currencyCt, currencyId
        //        ) :
        //        (0, 0);
        //
    ...
    notNullAddress(wallet)
    {
        ...
        (int256 depositedAmount,) = balanceTracker.logByBlockNumber(
            wallet, depositedBalanceTypeHash, currencyCt, currencyId, blockNumber
        );

        int256 settledBalanceAmount = amount.sub(depositedAmount);
        balanceTracker.set(wallet, settledBalanceTypeHash, settledBalanceAmount, currencyCt, currencyId);
    ...
    onlyAuthorizedService(wallet)
    {
        ...
        // Add to staged
        balanceTracker.add(wallet, stagedBalanceTypeHash, stageAmount, currencyCt, currencyId);
    ...
    public
    {
        ...
        unstageAmount = unstageAmount.clampMax(
            balanceTracker.get(msg.sender, stagedBalanceTypeHash, currencyCt, currencyId)
        );
        if (unstageAmount == 0)
            return;

        // Move from staged balance to deposited balance
        balanceTracker.transfer(msg.sender, stagedBalanceTypeHash, depositedBalanceTypeHash, unstageAmount, currencyCt, currencyId);
    ...
    {
        //        walletLocker.lockByProxy(lockedWallet, lockerWallet);
    ...
    //    public
    //    {
    //        walletLocker.unlock(msg.sender);
    //
    ...
    public
    {
        require(walletLocker.isLockedBy(lockedWallet, msg.sender));

        // Get sum of balances for locked wallet
        int256 amount = balanceTracker.sum(lockedWallet, allBalanceTypeHashes, currencyCt, currencyId);
        require(amount >= 0);

        // Zero locked wallet's balances
        balanceTracker.reset(lockedWallet, allBalanceTypeHashes, currencyCt, currencyId);

        // Add to staged balance of sender
        balanceTracker.add(msg.sender, stagedBalanceTypeHash, amount, currencyCt, currencyId);

        // Add to the store of seized wallets
        if (!seizedByWallet[lockedWallet]) {
            seizedByWallet[lockedWallet] = true;
            seizedWallets.push(lockedWallet);
        }
    ...
    public
    {
        ...
        amount = amount.clampMax(balanceTracker.get(msg.sender, stagedBalanceTypeHash, currencyCt, currencyId));
        if (amount <= 0)
            return;

        // Subtract to per-wallet staged balance
        balanceTracker.sub(msg.sender, stagedBalanceTypeHash, amount, currencyCt, currencyId);

        // Log record of this transaction
        transactionTracker.add(msg.sender, withdrawalTransactionTypeHash, amount, currencyCt, currencyId);
    ...
    //    returns (int256 amount, uint256 blockNumber, address currencyCt, uint256 currencyId)
    //    {
    //        return transactionTracker.getByIndex(wallet, withdrawalTransactionTypeHash, index);
    //    }
    ...
    //    returns (uint256)
    //    {
    //        return transactionTracker.count(wallet, withdrawalTransactionTypeHash);
    //    }
    ...
    //    {
    //        return transactionTracker.getByCurrencyIndex(
    //            wallet, withdrawalTransactionTypeHash, currencyCt, currencyId, index
    //        );
    //    }
    ...
    //    {
    //        return transactionTracker.countByCurrency(
    //            wallet, withdrawalTransactionTypeHash, currencyCt, currencyId
    //        );
    //    }
    ...
    private
    {
        bytes32 balanceHash = 0 < bytes(balanceType).length ? keccak256(abi.encodePacked(balanceType)) : depositedBalanceTypeHash;

        if (stagedBalanceTypeHash == balanceHash)
            balanceTracker.add(wallet, stagedBalanceTypeHash, amount, currencyCt, currencyId);

        else if (depositedBalanceTypeHash == balanceHash) {
            // Add to per-wallet deposited balance
            balanceTracker.add(wallet, depositedBalanceTypeHash, amount, currencyCt, currencyId);

            // Log record of this transaction
            transactionTracker.add(wallet, depositTransactionTypeHash, amount, currencyCt, currencyId);
        }
    ...
    private
    {
        ...
        stageAmount = stageAmount.clampMax(balanceTracker.get(wallet, depositedBalanceTypeHash, currencyCt, currencyId).add(
                balanceTracker.get(wallet, settledBalanceTypeHash, currencyCt, currencyId)
            ));
        if (stageAmount <= 0)
            return;
        ...
        int256 settledBalanceAmount = balanceTracker.get(wallet, settledBalanceTypeHash, currencyCt, currencyId);
        ...
        if (settledBalanceAmount >= stageAmount)
            balanceTracker.sub(wallet, settledBalanceTypeHash, stageAmount, currencyCt, currencyId);

        // Else settled will be zeroed and (stage amount - settled) is deducted from deposited
        else {
            balanceTracker.add(wallet, depositedBalanceTypeHash, settledBalanceAmount.sub(stageAmount), currencyCt, currencyId);
            balanceTracker.set(wallet, settledBalanceTypeHash, 0, currencyCt, currencyId);
        }
    }
