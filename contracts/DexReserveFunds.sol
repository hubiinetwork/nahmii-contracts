	...
	function claimAccrual(Accrual accrual) public {
		...
		if (walletInfoMap[msg.sender].accrualTarget == ACCRUAL_TARGET.ActiveBalance) {
			if (accrual.tokenAddress == address(0)) {
				...
				walletInfoMap[msg.sender].depositsHistory.push(DepositHistory(accrual.tokenAddress, walletInfoMap[msg.sender].depositsToken[accrual.tokenAddress].length - 1));
			}
