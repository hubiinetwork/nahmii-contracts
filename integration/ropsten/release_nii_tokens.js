...
(async () => {
    ...
    const network = provider.name;

    const operatorWallet = Wallet.fromMnemonic('weekend impose display trade degree bounce subway pink winner round decrease salon');
    operatorWallet.provider = provider;

    const revenueTokenManager = await contractFactory.create('RevenueTokenManager', network, operatorWallet);
