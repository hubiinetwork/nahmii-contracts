...
exports.mockOrder = async (operator, params) => {
    ...
    const order = exports.mergeDeep({
        party: {
            ...
            balances: {
                intended: utils.parseUnits('9599.8', 18),
                conjugate: utils.parseUnits('9.4', 18)
            },
            data: 'some_party_data'
        },
        placement: {
    ...
    const nonce = (walletNonces.has(order.party.wallet) ? walletNonces.get(order.party.wallet) : 0) + 1;
    walletNonces.set(order.party.wallet, nonce);

    order.party.nonce = utils.bigNumberify(nonce);

    const operatorSigner = exports.createWeb3Signer(operator);

    const walletSigner = (
        order.party.wallet === wallet.address ?
            exports.createEthutilSigner(wallet) :
            exports.createWeb3Signer(order.party.wallet)
    );
...
exports.hashOrderAsWallet = (order) => {
    const partyHash = cryptography.hash(
        order.party.wallet,
        order.party.data
    );
    ...
    return cryptography.hash(partyHash, placementHash);
};
