...
module.exports = function (glob) {
	...
	describe("PartnerFund", function () {
		...
		it(testCounter.next() + ": MUST FAIL [receiveEthers]: Not registered tag", async() => {
			try {
				await glob.web3PartnerFund.receiveEthers(dummyTag, { value: web3.toWei(0.2, 'ether') });
				assert(false, 'This test must fail.');
			}
			catch (err) {
				assert(err.toString().includes('revert'), err.toString());
		...
		it(testCounter.next() + ": MUST FAIL [receiveTokens]: Not registered tag", async() => {
			try {
				await glob.web3PartnerFund.receiveTokens(dummyTag, 2, glob.web3Erc20.address);
				assert(false, 'This test must fail.');
			}
			catch (err) {
				assert(err.toString().includes('revert'), err.toString());
		...
		it(testCounter.next() + ": MUST SUCCEED [receiveEthers]: add 1 ether to user A deposited balance", async() => {
			try {
				await glob.web3PartnerFund.receiveEthers(userATag, { from: glob.owner, value: web3.toWei(1, 'ether') });
			}
			catch (err) {
				assert(false, 'This test must succeed. [Error: ' + err.toString() + ']');
			}
		...
		it(testCounter.next() + ": MUST SUCCEED [receiveTokens]: 3 tokens added to A deposited balance", async() => {
			try {
			...
			try {
				await glob.web3PartnerFund.receiveTokens(userATag, 3, glob.web3Erc20.address, { from: glob.owner });
			}
			catch (err) {
				assert(false, 'This test must succeed. [Error: ' + err.toString() + ']');
			}
